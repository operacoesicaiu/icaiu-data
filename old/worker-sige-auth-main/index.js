...
async function removerDuplicados({
    ...
    const filtradas = [
        header,
        ...filtradasReverso.reverse()
    ];

    const removidos =
        rows.length - filtradas.length;

    secureLog(
        `${removidos} duplicados removidos.`
    );
...
async function run() {
    ...
    try {
        ...
        secureLog(
            `Reprocessando últimos ${DIAS_REPROCESSAR} dias.`
        );
        ...
        while (dataAtual <= ontem) {
            ...
            if (pedidos.length === 0) {
                ...
                secureLog(
                    `Nenhum pedido encontrado em ${dataBusca}`
                );
            ...
            secureLog(
                `${rowsFinal.length} registros adicionados (${dataBusca})`
            );
