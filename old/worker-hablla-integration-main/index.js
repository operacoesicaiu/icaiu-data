...
async function run() {
        const { 
            ...
            HABLLA_WORKSPACE_ID, HABLLA_BOARD_ID, SPREADSHEET_ID, HABLLA_TOKEN
        } = process.env;
    ...
    try {
        ...
        const sheetCliente = meta.data.sheets.find(s => s.properties.title === "Base Cliente");
        const hasBaseCliente = !!sheetCliente;
        const idBaseCliente = hasBaseCliente ? sheetCliente.properties.sheetId : null;
        ...
        while (page <= 500) {
            ...
            const rowsToInsert = cards.filter(c => new Date(c.created_at) >= seteDiasAtras).map(card => {
                ...
                const userName = (card.user && typeof card.user === 'object') ? (card.user.name || card.user.email || "") : "";
        ...
        while (clientPage <= maxPages) {
            try {
                const resClients = await axios.get(`https://api.hablla.com/v1/workspaces/${HABLLA_WORKSPACE_ID}/persons`, {
                    params: {
                        start_date: dIni,
                        end_date: dFim,
                        page: clientPage,
                ...
                const data = resClients.data?.results || resClients.data?.data || resClients.data || [];
                ...
                console.error(`Erro na página ${clientPage} de clientes:`, err.response?.status, err.response?.data);
                break;
            }
        }
        ...
        if (allClients.length > 0 && hasBaseCliente) {
            ...
            const rowsClientes = allClients.map(person => {
                ...
                let outrosCamposStr = '';
                const camposEspecificos = ['name', 'emails', 'phones', 'sectors', 'tags', 'custom_fields', 'users', 'id', 'created_at', 'updated_at', 'workspace', 'duplicate_keys', 'instagrams', 'facebooks', 'followers', 'sla_config_id', 'workspace_id'];
                const outrosCampos = [];
                ...
                Object.keys(person).forEach(key => {
                    if (!camposEspecificos.includes(key) && person[key] !== null && person[key] !== undefined) {
                ...
                return [
                    ...
                    whatsapp,
                    sanitize(emailsStr),
                    person.created_at ? new Date(person.created_at).toLocaleString('pt-BR').replace(',', '') : '',
                    person.updated_at ? new Date(person.updated_at).toLocaleString('pt-BR').replace(',', '') : '',
                    sanitize(sectorsStr),
                    sanitize(tagsStr),
            ...
            await axios.post(`https://sheets.googleapis.com/v4/spreadsheets/${SPREADSHEET_ID}/values/Base%20Cliente!A:A:append?valueInputOption=USER_ENTERED`, 
                { values: rowsClientes }, 
                { headers: gHeaders }
            );
            ...
            console.log("Aba 'Base Cliente' não encontrada. Pulando exportação de clientes.");
        }
