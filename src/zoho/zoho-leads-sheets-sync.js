  ...
  try {
    ...
    while (true) {
      ...
      try {
        ...
        if (
          err.response &&
          (err.response.status === 404 || err.response.data.code === 3100)
        ) {
    ...
    if (false && allProcessed.length > 0) {
      ...
      for (let i = 0; i < allProcessed.length; i += 500) {
        ...
        secureLog(
          `Lote enviado: ${i + batch.length} de ${allProcessed.length}`,
        );
    ...
    const dataBR = `${dia}/${String(dataReferencia.getMonth() + 1).padStart(2, "0")}/${ano}`;
    const dataDash = `${dia}-${String(dataReferencia.getMonth() + 1).padStart(2, "0")}-${ano}`;
