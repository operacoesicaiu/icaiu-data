...
function formatZohoValue(val) {
  ...
  if (typeof val === "object") {
    if (Array.isArray(val)) {
      return val
        .map((v) => (typeof v === "object" ? v.display_value || v.ID : v))
        .join(", ");
    }
...
async function run() {
  try {
    ...
    const sheetObj = sheetMeta.data.sheets.find(
      (s) => s.properties.title === REPORT_SHEET_NAME,
    );
    const sheetId = sheetObj.properties.sheetId;
    let currentMaxRows = sheetObj.properties.gridProperties.rowCount;

    const resR = await axios.get(
      `https://sheets.googleapis.com/v4/spreadsheets/${REPORT_SPREADSHEET_ID}/values/${safeSheet}!R:R`,
      { headers: gHeaders },
    );
    ...
    const dictRes = await axios
      .get(
        `https://sheets.googleapis.com/v4/spreadsheets/${REPORT_SPREADSHEET_ID}/values/'Dicionário'!A:B`,
        { headers: gHeaders },
      )
      .catch(() => ({ data: {} }));
    const dictionary = {};
    if (dictRes.data.values)
      dictRes.data.values.forEach((r) => (dictionary[r[0]] = r[1]));
    ...
    const finalData = zohoRecords.map((rec) => {
      ...
      const colAG = 0;
      const colAH = colR.includes("/")
        ? `${colR.split("/")[1]}/${colR.split("/")[2]}`
        : "";
    ...
    if (requiredRows > currentMaxRows) {
      ...
      await axios.post(
        `https://sheets.googleapis.com/v4/spreadsheets/${REPORT_SPREADSHEET_ID}:batchUpdate`,
        {
          requests: [
            {
              appendCells: {
                sheetId: sheetId,
                rows: Array(addCount).fill({ values: [] }),
    ...
    if (deleteFromRow <= allDates.length && allDates.length > 0) {
      ...
      await axios.post(
        `https://sheets.googleapis.com/v4/spreadsheets/${REPORT_SPREADSHEET_ID}/values/${rangeClear}:clear`,
        {},
        { headers: gHeaders },
      );
    }
    ...
    for (let i = 0; i < finalData.length; i += batchSize) {
      ...
      await axios.put(
        `https://sheets.googleapis.com/v4/spreadsheets/${REPORT_SPREADSHEET_ID}/values/${rangeUpload}?valueInputOption=USER_ENTERED`,
        { values: batch },
        { headers: gHeaders },
      );
    }
    ...
    secureLog(
      `ERRO: ${e.response ? JSON.stringify(e.response.data) : e.message}`,
      true,
    );
    process.exit(1);
  }
