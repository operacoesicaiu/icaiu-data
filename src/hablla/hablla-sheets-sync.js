...
async function run() {
  ...
  try {
    ...
    console.error(
      `>>> [ERRO CONTROLADO] status=${e.response?.status || "network"}`,
    );
    process.exitCode = 1;
  }
}
